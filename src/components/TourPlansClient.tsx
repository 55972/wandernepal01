            ...
            <div key={plan.id} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow">
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2 text-sm text-kaleo-charcoal/60">
                    <Clock className="w-4 h-4" />{plan.duration}
                  </div>
                  <div className="font-serif text-xl text-kaleo-charcoal">
                    {plan.price.currency} {plan.price.amount.toLocaleString()}
                    <span className="text-sm text-kaleo-charcoal/50">/person</span>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2 mb-4 text-xs">
                  <div>
                    ...
                    {plan.inclusions.slice(0, 3).map((item, i) => (
                      <div key={i} className="flex items-center gap-1 text-kaleo-charcoal/60"><Check className="w-3 h-3 text-green-500 flex-shrink-0" />{item}</div>
                    ))}
                  </div>
                  <div>
                    <p className="font-medium text-kaleo-charcoal mb-1">Excludes</p>
                    {plan.exclusions.slice(0, 3).map((item, i) => (
                      <div key={i} className="flex items-center gap-1 text-kaleo-charcoal/60"><X className="w-3 h-3 text-red-400 flex-shrink-0" />{item}</div>
                    ))}
                  </div>
                </div>
                <div className="mb-4">
                  <p className="font-medium text-kaleo-charcoal text-xs mb-2">Highlights</p>
                  <div className="flex flex-wrap gap-1">
                    {plan.highlights.slice(0, 4).map((h, i) => (
                      <Badge key={i} variant="outline" className="text-xs border-kaleo-terracotta/30 text-kaleo-terracotta">{h}</Badge>
                    ))}
                  </div>
                </div>
                <div className="flex gap-2">
                  <Button onClick={() => setExpandedPlan(expandedPlan === plan.id ? null : plan.id)}
                    variant="outline" className="flex-1 border-kaleo-terracotta text-kaleo-terracotta hover:bg-kaleo-terracotta hover:text-white text-sm">
                    {expandedPlan === plan.id ? 'Hide Itinerary' : 'View Itinerary'}
                  </Button>
                  ...
                  <div className="mt-4 pt-4 border-t border-kaleo-terracotta/10 space-y-3">
                    {plan.itinerary.map((day) => (
                      <div key={day.day} className="border-l-2 border-kaleo-terracotta pl-3">
                        ...
                        <p className="text-xs text-kaleo-charcoal/60">{day.activities.join(' • ')}</p>
                      </div>
                    ))}
                  </div>
                )}
